...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if ($isEdit) {
            ...
            $stmt->bind_param("ssssii", $name, $description, $imageURL, $formUrl, $status, $app['id']);
            ...
            // Insert new app
            $sql = "INSERT INTO artist_v2_apps (name, description, image_url, form_url, status, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, NOW(), NOW())";
            
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssi", $name, $description, $imageURL, $formUrl, $status);
